...
if(!$get_today){
	...
	$log_start_sql = "insert into soodang_pay(`allowance_name`,`day`,`mb_id`,`mb_no`,`benefit`,`curency`,`rate`,`mb_level`,`grade`,`mb_name`,`rec`,`rec_adm`,`origin_balance`,`origin_deposit`,`datetime`) values";
	$log_values_sql = "";
	...
	for($i = 0; $i < $order_list_row = sql_fetch_array($order_list_result); $i++){
		...
		$_clean_number_benefit  = clean_number_format($_benefit/$coin_curency,2);

		$rec = "staking bonus {$staking_bonus_rate}% : {$_clean_number_benefit} {$unit} ";
        // "Shop Bonus : {$clean_shop_benefit} {$shop_unit} {$over_benefit_log}";
		$benefit_log = "{$clean_number_goods_price} (PV) * ( {$staking_bonus_rate}% ){$over_benefit_log}";
		...
		$log_values_sql .= "('{$code}','{$bonus_day}','{$order_list_row['mb_id']}',{$order_list_row['mb_no']},{$_clean_number_benefit},{$coin_curency},{$staking_bonus_rate},{$order_list_row['mb_level']},{$order_list_row['grade']},
	...
	foreach($total_paid_list as $key=>$value){
		...
		$live_benefit = clean_number_format(($value['real_benefit'] * $live_bonus_rate / $coin_curency),2);
		...
		echo "<span class='title'>{$key}</span>{$value['sub_log']}<br>{$value['log']}<div style='color:orange;'>발생 수당 : {$value['total_benefit']} / ".$coin_curency."</div><div style='color:red;'>▶ 수당지급 : {$live_benefit} ";
