...
<div class="local_ov01 local_ov" style="display: flex; align-items: center">
    <a href="./adm.deposit_request.php?<?= $qstr ?>" class="ov_listall"> 결과통계 <?= $total_count ?> 건</a>
    <?
    // 현재 통계치
    ...
    <table class='regTb' id='table'>
        ...
        <thead>
            <tr>
                ...
                <th scope="col" width='5%'>입금처리금액(<?= $curencys[1] ?>)</th>
                ...
                <tr class="bg0">
                    ...
                    <td><?php
                        if ($row['coin'] == $curencys[0]) {
                            echo shift_auto($row['in_amt'], KRW_NUMBER_POINT);
                        } else echo shift_auto($row['amt'], COIN_NUMBER_POINT);
                        ?></td>
                    <td class='coin'><?= $row['coin'] ?></td>
                    <td><input type='text' class='reg_text input_amt_val' style='font-weight:600;color:blue;text-align:right' value='<?= shift_auto($row['amt'], $curencys[0]) ?>'></td>
        ...
        <? } ?></td> -->
            ...
            <?php
                if ($row['coin'] == $curencys[0]) {
                    $total_in_krw_amt += $row['in_amt'];
                } else $total_in_core_amt += $row['in_amt'];
                $total_inamt += $row['amt'];
            }
            if ($i == 0)
        ...
        <tfoot>
            <tr>
                <td>합계</td>
                <td colspan="4"></td>
                <td colspan="2" class="text-right"><?= shift_auto($total_in_krw_amt, $curencys[0]) ?> <?= $curencys[0] ?> <br><?= shift_auto($total_in_core_amt) ?> <?= $curencys[1] ?></td>
                <td class="text-right"><?= shift_auto($total_inamt) ?> <?= $curencys[1] ?></td>
                <td colspan="6"></td>
            </tr>
        </tfoot>
        </tbody>
